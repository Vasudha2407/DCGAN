...
class DCGAN(object):
    ...
    def face_dcgan_alexnet(self, x, scope_name='alexnet', if_age=False, reuse=False):
        with tf.variable_scope(scope_name, reuse=reuse) as scope:
            # 1st Layer: Conv (w ReLu) -> Pool -> Lrn
            conv1 = conv(x, 11, 11, 96, 4, 4, padding='VALID', name='conv1')
            pool1 = max_pool(conv1, 3, 3, 2, 2, padding='VALID', name='pool1')
    ...
    def train_dcgan_lsgan_transfer(self, source_img_227, source_img_128, imgs, true_label_fea_128, true_label_fea_64,
    ...
    def load(self, saver, prefix=None, model_num=None):
        print(" [*] Reading checkpoints...")
        saver.restore(self.sess, 'model/dcgan/dcgan.model-399999')
        return True
